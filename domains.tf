...
resource "aws_route53_record" "dev_frontend_record" {
  zone_id = data.aws_route53_zone.zone.zone_id
  name    = "dev.${var.personal_website_domain}"
  type    = "A"
  alias {
    name = aws_s3_bucket.dev_frontend_s3_bucket.website_domain
    zone_id = aws_s3_bucket.dev_frontend_s3_bucket.hosted_zone_id
    evaluate_target_health = false
  }
}
